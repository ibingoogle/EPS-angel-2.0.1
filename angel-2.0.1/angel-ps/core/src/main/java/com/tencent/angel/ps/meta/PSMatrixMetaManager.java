...
package com.tencent.angel.ps.meta;
...
public class PSMatrixMetaManager {
  ...
  public List<ParameterServerId> getPss(PartitionKey partitionKey, boolean sync)
    ...
    if (!sync) {
      ...
      List<ParameterServerId> psIds =
        context.getMaster().getStoredPss(partitionKey.getMatrixId(), partitionKey.getPartitionId());
  ...
  public PartitionLocation getPartLocation(PartitionKey partitionKey) throws ServiceException {
    ...
    for (int i = 0; i < size; i++) {
      psLocs.add(
        new PSLocation(psIds.get(i), context.getLocationManager().getPsLocation(psIds.get(i))));
    }

...
public class MasterClient {
   ...
   * @throws Exception rpc failed
   */
  public void createMatrices(List<MatrixContext> matrixContexts, long timeOutMS) throws Exception {
    ...
    while (true) {
      ...
      if (checkResponse.getStatus() == 0) {
        ...
        List<MatrixMetaProto> metaProtos = master
          .getMatrices(null, GetMatricesRequest.newBuilder().addAllMatrixNames(matrixNames).build())
          .getMatrixMetasList();
        for (int i = 0; i < size; i++) {
   ...
   * @throws ServiceException rpc failed
   */
  public void taskCountersUpdate(Map<String, String> counters, int taskIndex)
    throws ServiceException {
